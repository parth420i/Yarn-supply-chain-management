        ...
        if st.button("Run Predictive Model", type="primary"):
            ...
            with g_col1:
                if "carrier" in df.columns:
                    ...
                    fig_c = px.bar(carrier_delay, x="carrier", y="delay_flag", title="Actual Failure Rate per Carrier", color="carrier")
            ...
            with g_col2:
                if "source_city" in df.columns and "destination_city" in df.columns:
                    # Dynamically construct routing text natively without mutating the main math frame
                    plot_df = df.copy()
                    ...
                    fig_r = px.bar(route_delay, x="delay_flag", y="Route", orientation='h', title="Top 5 Most High-Risk Corridors", color="delay_flag", color_continuous_scale="Reds")
            ...
            cost_df = pd.DataFrame({"Execution Strategy": ["Baseline Freight Execution", "ML Expected-Value Intervention"], "Total Logistics Overhead (INR)": [baseline_cost, optimized_cost]})
            fig_cost = px.bar(cost_df, x="Execution Strategy", y="Total Logistics Overhead (INR)", color="Execution Strategy", color_discrete_sequence=["#e74c3c", "#2ecc71"], text="Total Logistics Overhead (INR)")
            ...
            if hasattr(clf, 'feature_importances_'):
                ...
                fig_imp = px.bar(imp_df, x="Relative Importance Weight", y="Feature Indicator", orientation='h', color_discrete_sequence=["#3498db"])
                ...
                if lr is not None:
                    ...
                    fig_imp = px.bar(imp_df, x="Relative Impact Vector", y="Feature Indicator", orientation='h', color_discrete_sequence=["#f39c12"], title="Logistic Regression Intercept Vectors")
                    fig_imp.update_layout(yaxis={'categoryorder':'total ascending'})
                    st.plotly_chart(fig_imp, use_container_width=True)
